type ElementSettings = [...string[]];
type Settings = [Style, ...string[]];
...
function newPage(props: Settings) {
    // The page itself:
    ...
    var elements = newComponent(props.shift() as Style, props);
...
function newComponent(style: Style, props: ElementSettings): HTMLElement[] {
    return COMPONENTS[style](props);
};

type ComponentFn = (props: ElementSettings) => HTMLElement[];

// I use a Look-Up-Table to reduce code duplication
const COMPONENTS: { [key: string]: ComponentFn } = {
    [Style.Title]: title,
    [Style.HeadingWithText]: headingWithText,
    [Style.Form]: form,
};

function title(props: ElementSettings) {
    var heading = document.createElement("h1");
    var headingText = props.shift();
    heading.style.fontFamily = "Century Gothic,CenturyGothic,AppleGothic,sans-serif"
    heading.innerText = headingText as string;
    document.body.appendChild(heading);
    return [heading];
};

function headingWithText(props: ElementSettings) {
    var heading = document.createElement("h1");
    var headingText = props.shift();
    heading.style.fontFamily = "Century Gothic,CenturyGothic,AppleGothic,sans-serif"
    heading.innerText = headingText as string;
    ...
    body.innerText = bodyText as string;
    document.body.appendChild(body);
    return [heading, body];
};

// Unimplemented
function form(props: ElementSettings) {
    var heading = document.createElement("h1");
    document.body.appendChild(heading);
    return [heading];
};

PAGES.forEach((page: Settings, idx) => {
    var div = newPage(page);
    var additions = PAGE_IMAGE_ADDITIONS[idx.toString()];
    if (additions) {
        additions.forEach((path: string) => {
